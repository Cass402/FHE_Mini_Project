...
use std::error::Error;
use tfhe::integer::{SignedRadixCiphertext, ServerKey};
...
fn deserialize_ciphertexts(encrypted_vector: &EncryptedVector) -> Vec<SignedRadixCiphertext> {
    encrypted_vector.data.iter().map(|data| {
...
fn serialize_ciphertexts(ciphertexts: Vec<SignedRadixCiphertext>) -> EncryptedVector {
    let data: Vec<Vec<u8>> = ciphertexts.iter().map(|ciphertext| {
        bincode::serialize(ciphertext).unwrap()
    }).collect();
    EncryptedVector { data, length: ciphertexts.len() }
}
...
pub fn compute_encrypted_sum(encrypted_vector: &EncryptedVector, server_key: &ServerKey) -> Result<SignedRadixCiphertext, Box<dyn Error>> {
...
pub fn compute_encrypted_mean(encrypted_vector: &EncryptedVector, server_key: &ServerKey) -> Result<EncryptedVector, Box<dyn Error>> {
    // Compute the sum
...
*/
pub fn compute_encrypted_category_counts(encrypted_categories: &HashMap<String, EncryptedVector>, server_key: &ServerKey) -> Result<HashMap<String, EncryptedVector>, Box<dyn Error>> {
...
pub fn run_biosample_analysis(encrypted_data: &HashMap<String, EncryptedVector>, server_key: &ServerKey) -> Result<HashMap<String, EncryptedVector>, Box<dyn Error>> {
    ...
    // Count blood types
    let blood_type_keys: Vec<String> = encrypted_data.keys().filter(|k| k.starts_with("blood_type_")).cloned().collect();

    if !blood_type_keys.is_empty() {
        let blood_type_data: HashMap<String, EncryptedVector> = blood_type_keys.iter().map(|k| (k.clone(), encrypted_data[k].clone())).collect();
