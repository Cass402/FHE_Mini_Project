...
impl BiosampleFHE {
    ...
    pub fn encrypt_f64_vector(&self, values: &[f64], scale: f64) -> EncryptedVector {
        ...
        // Encrypt each value
        let encrypted_data: Vec<Vec<u8>> = scaled_values.iter().map(|&v| {
            let ciphertext = self.client_key.encrypt_signed_radix(v, FHE_INT_BITS);
            bincode::serialize(&ciphertext).unwrap()
        }).collect();

        EncryptedVector { data: encrypted_data, length: values.len() }
    }
    ...
    pub fn encrypt_bool_vector(&self, values: &[bool]) -> EncryptedVector {
        ...
        // Encrypt each value
        let encrypted_data: Vec<Vec<u8>> = int_values.iter().map(|&v| {
            let ciphertext = self.client_key.encrypt_signed_radix(v, FHE_INT_BITS);
            bincode::serialize(&ciphertext).unwrap()
        }).collect();
        EncryptedVector { data: encrypted_data, length: values.len() }
    ...
    pub fn encrypt_categorical(&self, values: &[String]) -> EncryptedCategorical {
        ...
        let mut categories: Vec<String> = values.iter().cloned().collect::<std::collections::HashSet<_>>().into_iter().collect();
        ...
        let encrypted_vectors: Vec<EncryptedVector> = one_hot_vectors.iter().map(|v| self.encrypt_bool_vector(v)).collect();

        EncryptedCategorical { categories, vectors: encrypted_vectors }
    }
    ...
    pub fn save_keys(&self, client_key_path: &Path, server_key_path: &Path) -> Result<(), Box<dyn Error>> {
        // Save the client key
    ...
    pub fn load_keys(client_key_path: &Path, server_key_path: &Path) -> Result<Self, Box<dyn Error>> {
        // Load the client key
    ...
    pub fn encrypt_biosample_data(
        ...
        encrypted_data.insert(
            "age".to_string(),
            fhe.encrypt_f64_vector(&ages, scale),
        );
        ...
        encrypted_data.insert(
            "marker".to_string(),
            fhe.encrypt_bool_vector(&marker),
        );
